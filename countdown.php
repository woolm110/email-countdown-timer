...
class CountdownTimer
{
    ...
    public function __construct($settings)
    {
        ...
        $this->boxColor = $this->hex2rgba($settings['boxColor'], false, '#4dd1f2');
        $this->fontColor = $this->hex2rgba($settings['fontColor'], false, '#2175ba');
        ...
        $this->fontSettings['path'] = realpath($this->fontPath . $settings['font'] . '.ttf');
        ...
        $this->fontSettings['characterWidth'] = imagefontwidth($this->fontSettings['size']);
     ...
     * @source: https://gist.github.com/EgoistDeveloper/42c36b1389cffb1bf613f6f6ebefa47b
     * @return array|string
     */
    function hex2rgba($color, $opacity = false, $default = '#758491')
    {
        ...
        if ($match){
            ...
            if ($matches){
                ...
                // reorder array and hexadec to rgb
                return array_map('hexdec', array_values($matches));
            }
        }

        return $this->hex2rgba($default);
    }
    ...
    {
        imagefilledrectangle($image, 0, 0, $this->width, $this->height, imagecolorallocate($image, $this->boxColor[0], $this->boxColor[1], $this->boxColor[2])
        );
    }
